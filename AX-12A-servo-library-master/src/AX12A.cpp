...
{	
    ...
    while (availableData() > 0)
    {
	  ...
	  Serial.print("Position: "); Serial.print(Position_Long_Byte); Serial.print("   Register return: "); Serial.println(ID);
...
int AX12A::readRegister(unsigned char ID, unsigned char reg, unsigned char reg_len)
{
	...
	while (availableData() > 0)
	{
		//Serial.println("Available data is greater than 0");
		Incoming_Byte = readData();
		if ( (Incoming_Byte == 255) & (peekData() == 255) )
		{
			//Serial.println("Receieved start byte");
			readData();                            // Start Bytes
			readData();                            // Ax-12 ID
			readData();                            // Length
			if( (Error_Byte = readData()) != 0 )   // Error
