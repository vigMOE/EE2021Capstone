...
void setup()
{
  ...
  */
  ...
  delay(100);
  */
  Serial.println("Motor comms startup test...");
  id_Search();
  Serial.println("SEARCH FINISHED");
  Serial.println("BAUD SEARCH");
  baud_Search();
  Serial.println("BAUD SEARCH FINISHED");
  ...
  read_ID(6); //servo arm
  read_ID(2); //servo arm
  Serial.println("Startup test finished");
...
//LOCKING ARM SERVO MOTORS
  ...
  lock_onehund = ax12a.readPosition(100); //servo arm
  ax12a.move(100, lock_onehund);
  lock_ninety = ax12a.readPosition(90); //servo arm
  ax12a.move(90, lock_ninety);
  lock_twelve = ax12a.readPosition(12); //servo arm
  ax12a.move(12, lock_twelve);
  lock_eightteen = ax12a.readPosition(18); //servo arm
  ax12a.move(18, lock_eightteen);
  lock_six = ax12a.readPosition(6); //servo arm
  ax12a.move(6, lock_six);
  lock_two = ax12a.readPosition(2); //servo arm
  ax12a.move(2, lock_two);
  //Serial.println("-----------------------------");
...
void loop()
{
 ...
 //ax12a.turn(Back, RIGHT, 0);
  ...
  //LEFT WILL MAKE IT MOVE FORWARD (MOTOR 11)
 ...
 //ax12a.turn(Right, RIGHT, velocity);
  ...
  if(id_input == 90) {
    lock_ninety = position_input;
    ax12a.move(90, lock_ninety);
  }
  if(id_input == 100) {
    lock_onehund = position_input;
    ax12a.move(100, lock_onehund);
  }
  if(id_input == 12) {
    lock_twelve = position_input;
    ax12a.move(12, lock_twelve);
  }
  if(id_input == 18) {
    lock_eightteen = position_input;
    ax12a.move(18, lock_eightteen);
  }
  if(id_input == 6) {
    lock_six = position_input;
    ax12a.move(6, lock_six);
  }
  if(id_input == 2) {
    lock_two = position_input;
    ax12a.move(2, lock_two);
  }
...
//SEARCH FOR BAUD RATES ON DAISY CHAIN
void baud_Search()
{
   for(int i =0; i<254; i++)
  {
  return_i = ax12a.readRegister(i, 4u, 1u);
  if(return_i != -1)
  {
  Serial.print("Register: "); Serial.print(i); Serial.print("   Register return: "); Serial.println(return_i);
  }
